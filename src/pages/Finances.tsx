...
export default function Finances() {
  ...
  const [profitPct, setProfitPct] = useState(54);
  ...
  const salePrice = partnerPrice > 0 ? partnerPrice * (1 + profitPct / 100) : 0;
  const reposition = salePrice * 0.65;
  const profit = salePrice * 0.30;
  const expenses = salePrice * 0.05;
    ...
    <div className="px-4 pt-6 pb-4">
      ...
      <Tabs defaultValue="weekly" className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-muted rounded-xl h-10">
          ...
          <TabsTrigger value="simulator" className="text-xs rounded-lg data-[state=active]:bg-navy data-[state=active]:text-primary-foreground">
        ...
        <TabsContent value="simulator" className="mt-4">
          <div className="bg-card rounded-xl p-5 shadow-card space-y-4">
            <div>
              <Label>Precio Socia (lo que pagas a Price)</Label>
              <Input
                type="number"
                value={partnerPrice || ''}
            ...
            <div>
              <Label>% de ganancia que quieres</Label>
              <Input
                type="number"
                value={profitPct}
            ...
            {salePrice > 0 && (
              ...
              >
                <div className="bg-gradient-gold rounded-xl p-4 text-center">
                  ...
                  <p className="text-3xl font-bold text-accent-foreground">{formatCurrencyDecimals(salePrice)}</p>
                </div>
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-muted rounded-lg p-3">
                    <p className="text-[10px] text-muted-foreground">Reposición</p>
                    <p className="text-sm font-semibold">{formatCurrency(reposition)}</p>
                  </div>
                  <div className="bg-muted rounded-lg p-3">
                    <p className="text-[10px] text-muted-foreground">Ganancia</p>
                    <p className="text-sm font-semibold text-navy">{formatCurrency(profit)}</p>
                  </div>
                  <div className="bg-muted rounded-lg p-3">
